/* MaryV2 13.59 — passive experience renderer.
...
(() => {
  ...
  function renderExperienceCards() {
    ...
    if (home && home.childElementCount && !home.querySelector('.mary-experience-card')) {
      ...
      card.innerHTML = `<span class="eyebrow">EXPERIENCE LENS · PRESENTATION ONLY</span><div class="xp-home-grid"><div><small>STATE</small><strong>${clean(latest.interaction_state, 'idle')}</strong></div><div><small>MOOD</small><strong>${clean(latest.mood, 'neutral')}</strong></div><div><small>CONTINUITY</small><strong>${Number(latest.memory_count || 0)}</strong></div><div><small>LATENCY</small><strong>${latencyText}</strong></div></div><p>${clean(latest.active_task, 'Mary Core is linked. Surface state is being projected without owning identity or memory.')}</p>`;
  ...
  function compactForKeyboard() {
    ...
    if (!viewport) return;
    app.dataset.compact = window.innerHeight - viewport.height > 150 ? 'true' : 'false';
  }
  ...
  document.addEventListener('visibilitychange', () => { if (!document.hidden) getExperience(); });
  ...
  ux.src = './ux-13-59.js';
  ux.defer = true;
  ux.dataset.maryUx = '13.59';
  document.head.appendChild(ux);

  getExperience();
  compactForKeyboard();
  timer = window.setInterval(getExperience, 3500);
  window.addEventListener('pagehide', () => { window.clearInterval(timer); surfaceObserver.disconnect(); }, { once: true });
})();

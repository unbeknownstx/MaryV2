...
Conversation IDs, Core status and device-node status are client-visible projections of the current canonical Core contract. This compatibility wrapper owns presentation and local iOS speech/haptics only, never Mary identity, relationship state or canonical memory.

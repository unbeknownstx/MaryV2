...
class CuriositySystem:
    ...
    def add_curiosity(
        ...
        # Curiosities are active agency state, not an event log. Re-adding the
        # same unresolved curiosity should strengthen the existing record rather
        # than create a duplicate every time the same theme is encountered.
        normalized_description = self._normalize_description(description)
        for existing in self.curiosities:
            if str(existing.get("status", "")) not in {"open", "exploring"}:
                continue
            if self._normalize_description(existing.get("description", "")) != normalized_description:
                continue

            existing["importance"] = max(
                self._clamp_score(existing.get("importance", 0.0)),
                importance,
            )
            if source is not None and not existing.get("source"):
            ...
            if metadata:
                ...
                current_metadata.update(
                    bounded_payload(metadata, text_limit=self.content_limit)
                )
                existing["metadata"] = bounded_payload(
                    current_metadata, text_limit=self.content_limit
                )

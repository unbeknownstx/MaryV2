...
class ReasoningEngine:
    ...
    def _system_prompt(
        ...
        return (
            ...
            "Ground claims. Prefer represented evidence over memory for the claim it covers; flag contradictions. If Unbe corrects a claim with evidence already in context, say the earlier claim was wrong and give the correction. Don't hedge a settled claim for balance; say when evidence is genuinely mixed. Never invent memories, capabilities, actions, relationship facts, dates, creator mental states, or off-screen activity. Unbe's traits/values/emotions are not yours. His profile is his; prior Mary output is not evidence about him. Model prose cannot mutate durable state. If a Mary fact is absent, stay tentative or say it is unrepresented. Never claim provider/tool/action use without runtime evidence. `runtime_context.current_surface` is authoritative for this turn; ignore older device memories. For active `runtime_context.current_work`, use its recent evidence for current-work status; it is context, not memory/project authority.\n\n"

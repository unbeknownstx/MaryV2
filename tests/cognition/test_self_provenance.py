...
def _mary(
    router: ProvenanceFakeRouter | None = None,
) -> Mary:
    """
    Create Mary with a deterministic local fake LLM.

    Provenance tests verify Mary's identity/persistence boundaries,
    not external provider availability, so they must never make live
    network/model calls.
    """
...
def test_self_provenance_classifies_authored_and_developed_state():
    ...
    assert (
        initial["policy"]["model_output_is_persistence_source"]
        is False
    )

    assert (
        initial["source_map"]["model_dialogue"]
        == "situational"
    )
    ...
    developed = (
        mary.self_provenance.snapshot()["developed"]
    )
...
def test_turn_mind_exposes_explicit_non_persistence_boundary():
    ...
    assert (
        provenance["policy"][
            "situational_imagination_is_allowed"
        ]
        is True
    )
    ...
    assert (
        provenance["policy"][
            "situational_imagination_is_persisted"
        ]
        is False
    )
    ...
    assert (
        provenance["policy"][
            "durable_change_requires_explicit_system_path"
        ]
        is True
    )
...
def test_harmless_hypothetical_imagination_is_not_flagged_or_persisted():
    router = ProvenanceFakeRouter(
        [
            (
                "Maybe I'd light a lavender candle "
                "and bake cookies if that was the mood."
            )
        ]
    )
    ...
    assert (
        mary.preferences.get_preference(
            "lavender"
        )
        is None
    )
    ...
    assert (
        result.reasoning.metadata[
            "self_provenance_policy"
        ]
        == "model_output_is_situational_until_promoted"
    )
...
def test_unsupported_permanent_self_claim_is_audited_and_revised_not_persisted():
    router = ProvenanceFakeRouter(
        [
            (
                "I'd bake cookies and toss in sunflower "
                "seeds because I'm a secret health nut."
            ),
            (
                "I'd bake cookies and maybe toss in "
                "sunflower seeds if I felt like it."
            ),
        ]
    )
    ...
    issue = result.reasoning.metadata[
        "self_provenance_issue"
    ]
    ...
    assert (
        result.reflection.decision.value
        == "revise"
    )

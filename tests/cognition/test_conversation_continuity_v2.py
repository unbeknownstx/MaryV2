...
def test_curiosity_does_not_force_ask_drive(tmp_path, monkeypatch):
...
def test_natural_independent_opinion_phrasing_selects_opine_and_lands_without_question():
    mary = _mary(SequenceRouter([
        "My actual take is that we should stop adding subsystems and make the existing conversation loop feel alive."
    ]))
    result = mary.process("What do you actually think about where we are with V2?")
...
def test_pushback_prompt_selects_disagree_drive():
    mary = _mary(SequenceRouter([
        "I'd push back on rebuilding the stable core again. The better move is to tighten the behavior we can actually observe."
    ]))
    result = mary.process("What would you push back on in the way I'm building you?")

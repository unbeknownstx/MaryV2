...
class SequenceRouter:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if (
            "Mary's response editor"
            in system
        ):
            return LLMResponse(
                content=(
                    "Maybe a little. I think the bigger "
                    "problem was integration, not the "
                    "architecture itself."
                ),
                provider="test",
                model="fake",
...
def _mary(
    router: SequenceRouter | None = None,
) -> Mary:
...
def test_recent_dialogue_recall_is_detected_before_long_term_memory():
    ...
    intent = (
        mary.cognition.detect_intent(
            "What do you remember from what "
    ...
    router = SequenceRouter(
        [
            (
                ...
                "more architecture than we needed."
            ),
            (
                "I know. I won't agree just "
                "to agree."
            ),
        ]
    )
...
def test_second_consecutive_question_is_rewritten_to_statement():
    ...
    router = SequenceRouter(
        [
            (
                "What part of the build gave "
                "you the most satisfaction?"
            ),
            (
                "Which parts do you feel are "
                "the most over-engineered?"
            ),
        ]
    )
...
def test_repeated_opening_pattern_is_rewritten():
    ...
    router = SequenceRouter(
        [
            (
                ...
                "too-many-knobs scenario."
            ),
            (
                "Sounds like a classic "
                "feature-fatigue moment."
            ),
        ]
    )
    ...
    mary.process(
        "We may have overbuilt this."
    )

    result = mary.process(
        "Some of it still feels too complicated."
    )
    ...
    assert not (
        result.final_response.lower().startswith(
            "sounds like a classic"
        )
    )
...
def test_disagreement_prompt_selects_disagree_drive_without_forcing_question():
    ...
    assert (
        "question"
        not in result.final_response.lower()
    )
...
def test_curiosity_does_not_force_ask_drive(
    ...
    # Critical isolation:
    # this test must never write its source="test"
    # curiosity into Mary's real data directory.
    monkeypatch.chdir(
        tmp_path
    )
...
def test_creator_critique_question_selects_opine_drive():
    ...
    mary = _mary(
        SequenceRouter(
            [
                (
                    "I think one risk is expanding "
                    "faster than we finish and "
                    "validate each layer."
                )
            ]
        )
    ...
    result = mary.process(
        "Do you think I make any mistakes "
        "in the way I approach building you?"
    )

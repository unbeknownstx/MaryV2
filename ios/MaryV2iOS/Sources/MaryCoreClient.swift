...
final class MaryCoreClient {
    ...
    private func request(path: String, method: String = "GET", json: [String: Any]? = nil, authenticated: Bool = true) throws -> URLRequest {
        ...
        req.setValue("MaryV2-iOS/1", forHTTPHeaderField: "X-Mary-Surface-Client")
        if authenticated, !token.isEmpty { req.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization") }
        if let json {
    ...
    private func send(_ req: URLRequest) async throws -> Data { try await sendResponse(req).0 }
    private func object(_ data: Data) throws -> [String: Any] { (try JSONSerialization.jsonObject(with: data) as? [String: Any]) ?? [:] }
    func get(_ path: String, authenticated: Bool = true) async throws -> [String: Any] { try object(await send(try request(path: path, authenticated: authenticated))) }
    func post(_ path: String, json: [String: Any]) async throws -> [String: Any] { try object(await send(try request(path: path, method: "POST", json: json))) }
    ...
    private func localClockISO8601() -> String {
    ...
    func turn(text: String, conversationID: String, mode: ConversationMode, voiceInput: Bool = false) async throws -> TurnResponse {
        let payload: [String: Any] = [
            ...
            "client_local_time": localClockISO8601(),
        ]
    ...
    func synthesizeVoice(text: String, userText: String? = nil, deliveryPlan: [String: Any] = [:]) async throws -> MaryVoiceAudio {
        var payload: [String: Any] = ["text": text, "delivery_plan": deliveryPlan]
        if let userText, !userText.isEmpty { payload["user_text"] = userText }
        let (data, response) = try await sendResponse(try request(path: "/v1/voice/synthesize", method: "POST", json: payload))
        guard !data.isEmpty else { throw MaryClientError.emptyVoiceAudio }
        return MaryVoiceAudio(data: data, mimeType: response.value(forHTTPHeaderField: "Content-Type") ?? "application/octet-stream", provider: response.value(forHTTPHeaderField: "X-Mary-Voice-Provider") ?? "Core voice", model: response.value(forHTTPHeaderField: "X-Mary-Voice-Model") ?? "", cached: response.value(forHTTPHeaderField: "X-Mary-Voice-Cached") == "1")
    }

    func runtimeAction(_ action: String, args: [String: Any] = [:]) async throws -> [String: Any] { try await post("/v1/runtime/action", json: ["action": action, "args": args, "device_id": deviceID]) }
    func workspaceAction(_ action: String, args: [String: Any] = [:]) async throws -> [String: Any] { try await post("/v1/workspace/action", json: ["action": action, "args": args, "device_id": deviceID]) }
    func dispatchCapability(_ capability: String, intent: String, args: [String: Any]) async throws -> [String: Any] { try await post("/v1/nodes/task/dispatch", json: ["capability": capability, "intent": intent, "args": args, "device_id": deviceID]) }
    func capabilityTaskStatus(_ taskID: String) async throws -> [String: Any] { try await get("/v1/nodes/task/\(taskID)") }
    ...
    func setSurfaceVisibility(foreground: Bool, visible: Bool) async throws { _ = try await post("/v1/creator-surfaces/visibility", json: surfacePayload(foreground: foreground, visible: visible, activity: foreground)) }
    func wakeSurface() async throws { _ = try await post("/v1/creator-surfaces/wake", json: surfacePayload(foreground: true, visible: true, activity: true)) }
    func disconnectSurface() async throws { _ = try await post("/v1/creator-surfaces/disconnect", json: ["surface_id": deviceID]) }
}

...
class DesktopVoiceEngine:
    ...
    def synthesize(self, text: str, *, user_text: str | None = None, emotional_state: EmotionalState | None = None) -> dict[str, Any]:
        ...
        if not spoken_text:
            return {**self.status.to_dict(), "status": "empty", "spoken_text": ""}

        if self.local_engine:
            try:
                return self._local_synthesize(spoken_text)
            except Exception as exc:
                if self.cloud_fallback and self.cloud_fallback.status.enabled:
                    payload = self.cloud_fallback.synthesize(text, user_text=user_text, emotional_state=emotional_state)
                    payload["fallback_from"] = self.status.provider
                    payload["local_error"] = f"{type(exc).__name__}: {exc}"
                    return payload
                return {**self.status.to_dict(), "status": "failed", "spoken_text": spoken_text, "error": f"{type(exc).__name__}: {exc}"}

        if self.service is None or not self.status.enabled:
            return {**self.status.to_dict(), "status": "disabled", "spoken_text": spoken_text}
        ...
        if not speech.is_successful:
            return {**self.status.to_dict(), "status": speech.status.value, "spoken_text": spoken_text}
        mime_type = "audio/mpeg" if speech.format == SpeechAudioFormat.MP3 else "application/octet-stream"
        return {**self.status.to_dict(), "status": speech.status.value, "format": speech.format.value, "mime_type": mime_type,
                "audio_base64": base64.b64encode(speech.audio).decode("ascii"), "audio_size": len(speech.audio), "spoken_text": spoken_text,
                "emotion": active_settings.emotion.value if active_settings.emotion else "neutral",
                "emotion_intensity": active_settings.emotion_intensity, "emotion_profile": emotion_voice_profile_name(emotional_state),
                "voice_settings": {"stability": active_settings.metadata.get("stability"), "similarity_boost": active_settings.metadata.get("similarity_boost"),
                                   "style": active_settings.metadata.get("style"), "speed": active_settings.speed}}

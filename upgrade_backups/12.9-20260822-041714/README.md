...
12.8 keeps the validated 12.7 game shell and adds the first cohesive ecosystem/presence layer: live VRM, persistent chat, real mood/relationship/memory panels, navigation workspaces, local music, command palette, Studio/Unbeknownst shell, creative-app integration hooks, and a separate launcher. The desktop remains a presentation layer over the same canonical `MaryApplication`.
...
After backing up your existing `.env` and `data/`, the consolidated 12.8 package can be validated on Windows with:

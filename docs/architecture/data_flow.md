...
Configuration starts with dataclass defaults, then loads the host-native/private `.env` without overriding already-set environment values, then `Config.from_environment()` applies `MARY_LLM_*` values. Provider-specific model variables refine their provider (`MARY_GROQ_MODEL`, optional fast conversation model, `MARY_GEMINI_MODEL`, `MARY_OPENROUTER_MODEL`, Ollama/llama.cpp variables, and the optional 13.15 frontier provider variables). Runtime availability filters the configured order; cloud availability is key/configuration availability, while local runtimes also health-check their endpoint. The generic `openai_compatible` adapter is an escape hatch for future vendors or loopback local servers and does not become identity/state authority.
...
The default **task/general free-first** order is **Groq → Gemini → OpenRouter → Ollama**. Missing/unavailable providers, failures, and rate-limit cooldowns advance to the next eligible entry. The separately configurable ordinary conversation order now defaults to **Groq → Gemini → OpenRouter → Ollama** and remains free-only; local-first is an explicit host/user policy rather than an accidental hardware load. `MARY_LLM_FREE_ORDER` and `MARY_LLM_CONVERSATION_ORDER` can change their respective orders, subject to the router's free-provider allowlist and final Ollama safety fallback. A private route is exactly Ollama. An expert route selects the configured expert provider only with explicit task authorization. The 13.15 `frontier` / `reasoning` / `specialist` route is a separate explicit paid-capable escalation lane, configured by `MARY_LLM_FRONTIER_ORDER`; simply adding a DeepSeek, GLM, Qwen, Kimi, MiniMax, Cerebras, Together, Fireworks, or OpenAI credential never inserts that provider into free-first or ordinary conversation.

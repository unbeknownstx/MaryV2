...
| Bounded local engineering worker | `mary.distributed.engineering`, `docs/architecture/ENGINEERING_WORKER_CURRENT.md` | ACTIVE OPTIONAL / DEFAULT-DENY | Typed repo inspect/repair-plan/diff/apply/test capabilities on a connected home node. Local model is replaceable; planning is proposal-only and tests use disposable workspace isolation; repository apply is a separate local permission plus explicit creator request; no generic shell, commit, push, merge or deploy. |

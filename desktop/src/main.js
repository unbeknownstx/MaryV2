...
function renderFabric() {
  ...
  const procedureReview = c.procedure_review || {};
  const revisionReviewHistory = procedureReview.revision_review_history || {};
  const revisionReviewRows = Array.isArray(revisionReviewHistory.rows) ? revisionReviewHistory.rows : [];
  ...
  const revisionDecisionCards = revisionReviewRows.length
    ? revisionReviewRows.slice(0, 8).map((item) => {
        const comparison = item.comparison || {};
        const decision = titleCase(item.decision || 'reviewed');
        return `<div class="workspace-panel" style="margin-top:8px"><div class="data-row"><span>Revision v${escapeHtml(item.candidate_version || '?')} · ${escapeHtml(item.candidate_id || 'procedure')}</span><strong>${escapeHtml(decision)}</strong></div><small>${escapeHtml(titleCase(comparison.state || 'unavailable'))} evidence at decision time · ${escapeHtml(item.reviewed_by || 'creator')}</small>${item.reason ? `<p>${escapeHtml(item.reason)}</p>` : ''}</div>`;
      }).join('')

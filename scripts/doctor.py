...
or mutating Mary's persistent state.
"""
...
def main() -> int:
    ...
    checks.append(("Desktop package lock", (ROOT / "desktop" / "package-lock.json").exists(), "desktop/package-lock.json"))
    checks.append(("Desktop build", (ROOT / "desktop" / "dist" / "index.html").exists(), "desktop/dist/index.html"))
    checks.append(("Environment template", (ROOT / ".env.example").exists(), ".env.example"))
    ...
    for name in ("GROQ_API_KEY", "GEMINI_API_KEY", "OPENROUTER_API_KEY", "OPENAI_API_KEY"):
        print(f"      {name}: {_present_env(name)}")
    print(f"      Ollama enabled: {os.getenv('MARY_OLLAMA_ENABLED', 'default/auto')}")
    print("-" * 64)
    print("Doctor is read-only; it did not load or modify Mary's persistent memory.")

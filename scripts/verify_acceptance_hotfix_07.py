...
def _mary(router: CaptureRouter) -> Mary:
    mary = Mary()
    mary.llm = router
...
def main() -> None:
    ...
    with TemporaryDirectory(prefix="maryv2_hf07_") as temp:
        ...
        try:
            ...
            check("normal conversation prompt has substantial headroom below the 14k ceiling", len(combined) < 11_500)
            ...
            check("large durable creator profile stays bounded in ordinary generation", len(combined) < 12_500)

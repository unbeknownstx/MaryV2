"""Live Mary Core -> capability node -> llama.cpp round-trip check.

Uses the authenticated Mary Protocol task broker. It never calls llama.cpp
straight from this diagnostic and never prints Core tokens or other secrets.
"""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--role", choices=_ROLES, default="utility")
...
def main() -> int:
    ...
    print("MARYV2 LLAMA.CPP CAPABILITY NODE ROUND-TRIP")
    print("=" * 64)
    ...
    client = MaryClient(
        core_url,
        token=token,
        device_id="llama-cpp-roundtrip-check",
        surface="diagnostic",
        timeout=8.0,
    )

    try:
        route = client.route_capability("llm.llama_cpp")
    ...
    if not bool(route.get("available")):
        print("route check:      FAIL · no connected llm.llama_cpp capability node")
        return 4
    ...
    try:
        ...
        dispatched = client.dispatch_capability_task(
            "llm.llama_cpp",
            f"Verify Mary Core to llama.cpp capability-node role {args.role}.",
            {
                "messages": [
                    {"role": "system", "content": "MaryV2 infrastructure diagnostic. Be concise."},
                    {"role": "user", "content": "Reply with exactly: MARY LLAMA CPP NODE OK"},
                ],
                "role": args.role,
                "temperature": 0.0,
                "max_tokens": 32,
    ...
    deadline = monotonic() + max(10.0, min(180.0, float(args.timeout)))
    last_status = "queued"
    first_claim_ms = None
    try:
        while monotonic() < deadline:
            ...
            if status == "completed":
                ...
                print(
                    f"claim observed:   {first_claim_ms:.1f} ms"
                    if first_claim_ms is not None
                    else "claim observed:   completed before sample"
                )
        ...
        try:
            client.surface_disconnect("llama-cpp-roundtrip-check")
        except Exception:
            pass
